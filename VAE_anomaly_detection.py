...
class VAEAnomalyDetection(pl.LightningModule, ABC):
    ...
    def forward(self, x: torch.Tensor) -> dict:
        ...
        x = x.unsqueeze(0)                                                                                              # shape: [1, batch_size, input_size], unsqueeze to broadcast input across sample dimension (L)
        # For each x in the batch, we have L samples. We calculate the log_like of this x from the L samples, by each of x's elements, and then .mean() by elements for each of the x(s).
        log_lik = Normal(pred_result['recon_mu'], pred_result['recon_sigma']).log_prob(x).mean(dim=0)                   # shape: from [L, batch_size, input_size] to [batch_size, input_size], average over sample dimension
        # we want the probability approach 1, so we want the log probability approach 0 from NEGATIVE values.
        log_lik = log_lik.mean(dim=0).sum()                                                                             # shape: from [input_size] to [1]

        # For each of x i in the batch, z is generated.
        kl = kl_divergence(pred_result['latent_dist'], self.prior).mean(dim=0).sum()

        loss = lambdaVAEAnomaly * kl - log_lik
    ...
    def predict(self, x) -> dict:
        ...
        recon_mu = recon_mu.view(self.L, *x.shape)                                                                      # shapr: [L, batch_size, input_size]
        recon_sigma = recon_sigma.view(self.L, *x.shape)                                                         # shapr: [L, batch_size, input_size]
    ...
    # For eacg batch, we do the training.
...
class VAEAnomalyTabular(VAEAnomalyDetection):
    ...
    def make_decoder(self, latent_size, output_size):
        ...
        return nn.Sequential(
            ...
            nn.Linear(32, 64),
            nn.ReLU(),
            nn.Linear(64 , output_size * 2)                                                                    # times 2 because this is the concatenated vector of reconstructed mean and variance
        )
